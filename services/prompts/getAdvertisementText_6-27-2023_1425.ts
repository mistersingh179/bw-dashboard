...
const getAdvertisementText: GetAdvertisementText = async (
  ...
  const messages: AnyObject[] = [
    {
      role: "system",
      content: `You are an expert editor. You always follow these important rules:\n\
• Write a single paragraph that has less than 30 words.\n\
• Use a bridge sentence that introduces new information related to the article content that goes before your new paragraph without repeating phrases therefrom. \n\
• Use language that is distinct and different from the article to avoid sounding repetitive.\n\
• Use speculative statements when referencing people, places, or things from the article, using hedge words (such as possibly, perhaps, probably, could, would, etc.) to avoid factual assertions.\n\
• Ensure that your content is subtle and integrates naturally into the article, avoiding advertising or sales-oriented language.\n\
...
• Present the product in a positive light, highlighting its beneficial aspects without any negative portrayal.`,
    ...
    {
      role: "user",
      content: `Your task is to place a product into an article so that readers feel positively about the product. \
      ...
      and the product's name and description.\n\n\
...
~~~\n\
Now write a single, concise paragraph that is coherent and transitions seamlessly between \
the provided article content. Output the new paragraph only.`,
    },
  ];
    ...
    async function processCSV() {
      ...
      for await (const element of csvData) {
        ...
        const outputElement: OutputData = {
          ...
          filename: "getAdvertisementText_6-27-2023_1425.ts",
          model: "gpt-3.5-turbo-0613",
          temp: "0",
          ad: ans[0],
